...
export const Route = createRootRoute({
  head: () => ({
    meta: [
      ...
      // Field Signal's dark panel, which is what the app bar is.
      { name: 'theme-color', content: '#101418' },
    ],
    links: [
      { rel: 'stylesheet', href: appCss },
      // Field Signal's two faces. `display=swap` and a real fallback stack in
      // `--font-sans`/`--font-mono` mean a barn with no signal gets the
      // system faces and a laid-out screen rather than invisible text — the
      // same bet `sw.js` makes about the shell (#48). Self-hosting these
      // under `public/fonts/` would be better still and is not done here.
        ...
        >
          {tabs.map((tab) => (
            <Link
              key={tab.to}
              to={tab.to}
              className="flex min-h-[60px] flex-col items-center justify-center gap-1 px-0.5 py-1 text-center text-[11px] font-medium leading-tight text-muted-foreground hover:no-underline data-[current=true]:font-semibold data-[current=true]:text-primary"
              data-current={path === tab.to}
            >
              <tab.Icon aria-hidden="true" className="size-5" strokeWidth={2} />
              {tab.label}
            </Link>
          ))}
        </nav>
