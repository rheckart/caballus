...
Sixteen tables. `orgs`; `idempotency_keys` (bookkeeping, not domain); Better Auth's `user`, `session` and `verification`, which alone carry no `org_id` and no policy, because identity is not org-scoped and Better Auth queries outside `forOrg`; and `volunteers`, `volunteer_accounts`, `volunteer_roles`, `volunteer_consents`, `release_versions`, `release_signatures`, `medication_authority`, `audit_entries`, `spaces`, `horses` and `horse_space_assignments`, which carry both like everything else will. `src/server/api/roster.test.ts` and `src/server/api/horses.test.ts` assert the policies against the catalogue, so a future table added without `enableRLS()` fails a test rather than leaking.
