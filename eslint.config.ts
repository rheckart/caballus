...
export default [
  {
    ignores: [
      ...
      'dist/**',
      'node_modules/**',
      // Sibling worktrees the orchestration tooling checks out inside this
      // repo for concurrently-running tickets (already excluded from git via
      // .git/info/exclude, but that file is local and ESLint's file scan
      // does not consult it) — each carries its own tsconfig.json, which
      // otherwise defeats typescript-eslint's tsconfigRootDir inference.
      '.claude/**',
