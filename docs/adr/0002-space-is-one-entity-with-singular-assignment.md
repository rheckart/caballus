...
So there is a single **Space** entity with a *kind* — stall, field, barn — composed of one or more named units, and a horse is assigned to exactly **one** Space per kind. `2 & 3` and `All of C + D` are single spaces with compound names, not sets.
...
**A many-to-many between horse and stall** was the first proposal in the #10 grilling and was rejected on the rescue's own framing — *"horses do not have 2 assigned Fields"* — and on how the work addresses it. You do not muck half of Blue's stall, and hay goes into a space rather than into a letter. A set-of-units model invites code that treats each unit separately, which is wrong for every joined case.
...
Per-area checklist items address Spaces, which is what the brief's separation of *muck stalls* from *muck paddocks* already implies: one entity, two kinds.
