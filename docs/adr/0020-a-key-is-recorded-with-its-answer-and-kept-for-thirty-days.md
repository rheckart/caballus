...
amended-by: #28, on what "route" means in the digest — the path that was sent, decoded, with its query, and never the pattern that matched it; #30, on what a write may answer with, so that a replay is the first answer rather than its shape; #31, on what the digest does with a value that is not a plain object, an array or a string — a Date, a bigint, or a shape nobody has decided a wire meaning for
---

# A key is recorded with its answer, kept for thirty days, and a changed request under it is refused

> **Amended on what the digest does with a Date, a bigint, or neither (#31).** `Object.entries(new Date())` is `[]` — every date canonicalised to the same `{}` as every other one, silently, so two care events differing only in when they happened digested identically and the second replayed the first's response. A `bigint` was worse: `JSON.stringify` refuses one outright, so it 500'd before the key was ever claimed and a retry met the same throw forever. Both are fixed by encoding what the value says rather than reading its own properties; everything else — a Map, a Set, a class instance — is refused, loudly, because nothing has decided what it means on the wire. See _A value the digest cannot see, refused rather than guessed at_ below.
>
> **Amended on what a write may answer with (#30).** Only a stored answer's status and body are kept, and the rest of it was rebuilt as `application/json` on the strength of a comment saying `json` is the only builder — which the 204 in the wrapper's own tests already disproved. A replay was therefore a JSON-shaped approximation of the first answer rather than the first answer. The claim is a type now: a mutation handler returns an `ApiResponse`, and only `src/server/api/answer.ts` makes one — `json` and `noContent` for a handler, `rebuild` for a stored answer, which is the same function answering the first attempt and the replay. See _A repeat gets the first response_ below.
>
> **Amended on what the digest's "route" is (#28).** It was built from the _registered pattern_, which is one string for every horse in the barn — so a key spent on `/horses/alfie/observations` and then on `/horses/bramble/observations` digested identically, and the second was answered 201 with alfie's response while nothing recorded it. Latent only because `/day` takes no parameters, and every mutation this domain needs is parameterised. The concrete path is what is digested, stored and logged now; the section below says so and says what became of the query string.
...
**The row is written inside the same `forOrg(...).run()` transaction as the work**, which is the guarantee and not an implementation detail: a key that outlived its rolled-back effect would turn every retry into a permanent silent failure — the write never happens and the phone is told it did. The claim is made _before_ the work rather than after, so that two attempts in flight at once serialise on the primary key instead of racing; the second blocks until the first commits or rolls back, and then sees the truth.

**This lives in the `mutation` wrapper, which opens the transaction and hands the handler the scoped handle.** ADR 0016 rejects an invariant every future handler has to remember, and _remember to record your key_ is precisely that. A handler receives `db` already inside the transaction that holds its key; there is no arrangement of a handler that opts out.

## A repeat gets the first response; a _different_ request under the same key gets a 409
...
### A value the digest cannot see, refused rather than guessed at (#31)

`canonical` reads a value it decides is a plain object with `Object.entries`, and a `Date` keeps every field it has in an internal slot — `Object.entries(new Date())` is `[]`, the same `[]` for every date that has ever existed. Two medication entries differing only in `administeredAt`, sent under one key by a client bug or a reused id: both canonicalised to the same `{}`, the fingerprints matched, and the second was answered 201 with the first entry's response while nothing recorded it — the exact confident lie this digest exists to prevent, told by silently reading the wrong question of a value that had already answered a different one. `z.coerce.date()` produces exactly this shape, and nothing in ADR 0016's day-boundary ban stops it: that ban is on the `Date` **identifier**, and a schema author reaches for `z.coerce.date()` without ever writing it.

**A date is now encoded as what it says**, not read for properties it does not have: `['Date', date.toISOString()]`, canonicalised through the same array path as everything else — tagged rather than a bare ISO string, so a date field and a string field that happens to hold the same ISO text stay distinguishable. `Object.prototype.toString.call(value)` does the detecting, not `instanceof Date`, so this file never writes the banned identifier either; the ban is for deriving a day boundary, and reading a date a schema already produced is a different question asked with the same word.

**A `bigint` is not an object at all** — `typeof 3n` is `'bigint'` — but `JSON.stringify` refuses one outright, a `TypeError` thrown before the key is ever claimed, so a retry meets the identical throw forever rather than the 409 a genuine conflict gets. Encoded the same way as a date, tagged: `['BigInt', value.toString()]`.

**Everything else that is an object but not a plain one is refused**, loudly: a `Map`, a `Set`, a class instance, a `RegExp`. None of them has a wire meaning anybody has decided, and encoding one wrong is the same failure this whole section is about, only rediscovered later. `isPlainObject` checks the prototype — `Object.prototype` or `null`, and nothing else — and anything that fails it throws `UnfingerprintableValueError` before `mutation` claims the key, which is the same shape the `bigint` bug already had and is accepted here rather than fixed: this is an authoring mistake in a hand-written Zod schema (ADR 0007), not a shape a real request should produce, and a loud failure — reported, in the same `onError` 500 path every other unhandled throw takes — at the first schema that makes the mistake is cheaper than a silent collision nobody notices at 6am. `Date` and `bigint` get the softer treatment because encoding them is lossless and obvious; nothing else on this list has an obvious encoding to reach for instead.

**A 4xx answer from a handler is recorded like any other.** It is a deliberate answer, and replaying it is correct: the retry gets the same rejection instead of a second attempt at something the server already refused. A handler that wants nothing recorded **throws**, and the transaction takes the key with it — which is also the only way a handler should produce a 5xx, since `onError` is the 500 path in this application. A handler that _returns_ a 5xx instead of throwing would have it recorded, and every retry would replay it forever. The wrapper does not second-guess that: a rule that quietly discards a handler's own response is worse than a convention the one door already follows.
...
**The digest is computed over the parsed body, not the bytes.** Fields the schema strips do not change it. That is the behaviour a retrying client needs, and it means a client sending a genuinely different _unknown_ field under a repeated key gets the first response. When a schema change makes an ignored field meaningful, it becomes part of the digest by becoming part of the schema — which is the right coupling, but it is a coupling.
...
**A field that is genuinely a two-element array `['Date', '...']` or `['BigInt', '...']` digests the same as a tagged value (#31).** The tag buys separation from a plain string or number holding the same text, not from an array shaped like the tag itself — the same class of low-probability collision this digest already accepts for every other type, where nothing distinguishes `true` from the string `"true"` beyond `JSON.stringify`'s own quoting. No schema in this domain has a field shaped that way today.
